...
#Esta parte es para cojer la predicción de Cassandra  
...
cluster = Cluster(['localhost'], port=9042)  # Especificar las direcciones IP de los nodos de Cassandra
session = cluster.connect('agile_data_science')  # Nombre del keyspace
...
def classify_flight_delays_realtime_response(unique_id):
  ...
  query = "SELECT * FROM flight_delay_classification_response WHERE \"UUID\"='"+unique_id+"'"
  prediction = session.execute(query).one()
